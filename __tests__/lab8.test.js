...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    let buttonInner = await button.getProperty('innerText');

    let textValue = await buttonInner['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let is20 = await cartText['_remoteObject'].value;
    expect(is20).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartText = await cartCount.getProperty('innerText');
    let is0 = await cartText['_remoteObject'].value;
    expect(is0).toBe("0");
